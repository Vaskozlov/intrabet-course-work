...
- **Коэффициенты и расчеты**: Коэффициенты определяются администратором или алгоритмом на основе статистики. Расчет выигрышей: выигрыш = ставка * коэффициент.
...
- **Прецедент "Просмотр списка событий"**:
  ...
  - Основной поток: 1. Пользователь открывает страницу событий (страница с таблицей или карточками). 2. Frontend запрашивает список через API. 3. Backend извлекает данные из БД, фильтрует (активные/завершенные). 4. Отображение: название, дата, коэффициенты, статус.
...
- **Прецедент "Размещение ставки"**:
  ...
  - Основной поток: 1. Пользователь выбирает событие и исход на детальной странице. 2. Ввод суммы (валидация на frontend). 3. API-запрос. 4. Backend проверяет баланс, сохраняет в таблицу Bets. 5. Подтверждение с расчетом потенциального выигрыша.
...
- **Прецедент "Пополнение баланса"**:
  ...
  - Основной поток: 1. Пользователь открывает профиль. 2. Выбирает "Пополнить" и вводит сумму (симуляция, без реальных платежей). 3. API. 4. Backend обновляет баланс в БД.
...
- **Прецедент "Просмотр истории ставок"**:
  ...
  - Основной поток: 1. Переход в раздел "Мои ставки". 2. API. 3. Backend извлекает из Bets. 4. Отображение: событие, сумма, исход, статус (выигрыш/проигрыш).
...
- **Прецедент "Расчет результатов"**:
  ...
  - Основной поток: 1. Admin выбирает событие. 2. Вводит фактический исход (выбирает события из списка вариантов). 3. API. 4. Backend: query Bets, расчет выигрышей, транзакции в БД.
...
- **Прецедент "Редактирование события"**:
  ...
  - Основной поток: 1. Выбор события. 2. Редактирование полей (дата, место, тип). 3. API. 4. Backend обновляет в БД.
...
- **Прецедент "Управление пользователями (блокировка)"**:
  - Актер: Администратор.
  - Предусловия: Авторизован.
  - Основной поток: 1. Просмотр списка пользователей. 2. Выбор пользователя. 3. Установка статуса "blocked". 4. Update в БД.
...
- **Прецедент "Генерация отчета по событиям"**:
  - Актер: Администратор.
  - Предусловия: Авторизован.
  - Основной поток: 1. Выбор параметров (дата, тип). 2. API. 3. Backend агрегирует данные. 4. Отображение в таблице или экспорт PDF (Angular).
