...
Информационная система (ИС) для букмекерской конторы на университетские события необходима для автоматизации процессов, которые в ручном режиме (например, через Google Forms или чаты) были бы неэффективными, подверженными ошибкам и ограниченными по масштабу. В университете ИТМО, где тысячи студентов участвуют в событиях, ручное управление ставками привело бы к хаосу: задержкам в расчетах, потере данных и низкой вовлеченности.
...
- **Usability**: Интуитивный интерфейс (Angular с responsive design); поддержка мобильных устройств.
- **Совместимость**: Работает в браузерах Chrome, Firefox; интеграция с PostgreSQL.
- **Поддержка**: Логирование ошибок; документация API.

### 4. Модели основных прецедентов и их описание (расширенная версия)

- **Прецедент "Регистрация пользователя"** (как ранее, но с углублением):
  - Актер: Гость (неавторизованный пользователь).
  - Предусловия: Пользователь не авторизован; система доступна.
  - Основной поток: 1. Гость переходит на страницу регистрации (Angular-компонент). 2. Вводит данные: логин, пароль, email, возможно, ФИО для университетского контекста. 3. Frontend валидирует данные (например, проверка на уникальность логина через API-запрос). 4. Backend (Jakarta EE servlet или EJB) сохраняет пользователя в БД (PostgreSQL, с хэшированием пароля bcrypt). 5. Автоматическая авторизация с выдачей JWT-токена.
  ...
  - Расширения: Подтверждение email через отправку ссылки (интеграция с email-сервисом, если доступно на helios).

- **Прецедент "Авторизация пользователя"**:
  - Актер: Гость.
  - Предусловия: Пользователь зарегистрирован, но не авторизован.
  - Основной поток: 1. Гость вводит логин и пароль на форме авторизации (Angular). 2. Frontend отправляет запрос на backend (REST API endpoint /login). 3. Backend проверяет credentials в БД, генерирует JWT-токен при успехе. 4. Frontend сохраняет токен в localStorage и перенаправляет на главную страницу.
...
- **Прецедент "Создание события"** (как ранее, но углубленно):
  - Актер: Администратор.
  - Предусловия: Авторизован как admin; JWT-токен валиден.
  - Основной поток: 1. Admin открывает форму создания (Angular-компонент с полями: название, дата, время, тип события — спорт/академия/культура, участники, возможные исходы в JSON-формате). 2. Ввод коэффициентов (ручной или алгоритмический расчет на backend). 3. Frontend отправляет данные на API (/events/create). 4. Backend валидирует (дата в будущем), сохраняет в таблицу Events (PostgreSQL). 5. Уведомление о создании (опционально, push в Angular).
  - Альтернативный поток: Если исходы некорректны (сумма вероятностей !=1) — автоматическая корректировка коэффициентов.
  - Исключительный поток: Конфликт дат с существующими событиями — ошибка.
  - Постусловия: Событие добавлено, visible для пользователей; лог в БД.
  - Расширения: Добавление медиа (фото события) — загрузка файлов на сервер helios.
...
- **Прецедент "Просмотр списка событий"**:
  ...
  - Основной поток: 1. Пользователь открывает страницу событий (Angular-компонент с таблицей или карточками). 2. Frontend запрашивает список через API (/events/list?filter=date&type). 3. Backend извлекает данные из БД (JPA query), фильтрует (активные/завершенные). 4. Отображение: название, дата, коэффициенты, статус.
  ...
  - Расширения: Пагинация для большого количества событий (Angular Material table).

- **Прецедент "Размещение ставки"** (как ранее, углубленно):
  - Актер: Пользователь.
  - Предусловия: Авторизован; событие активно (не завершено); баланс >= сумма ставки.
  - Основной поток: 1. Пользователь выбирает событие и исход на детальной странице (Angular). 2. Ввод суммы (валидация на frontend). 3. API-запрос (/bets/place). 4. Backend проверяет баланс (транзакция в PostgreSQL: update Users set balance = balance - amount), сохраняет в таблицу Bets. 5. Подтверждение с расчетом потенциального выигрыша.
...
- **Прецедент "Пополнение баланса"**:
  ...
  - Основной поток: 1. Пользователь открывает профиль (Angular). 2. Выбирает "Пополнить" и вводит сумму (симуляция, без реальных платежей). 3. API (/balance/deposit). 4. Backend обновляет баланс в БД (транзакция для атомарности).
...
- **Прецедент "Просмотр истории ставок"**:
  ...
  - Основной поток: 1. Переход в раздел "Мои ставки" (Angular таблица). 2. API (/bets/history?userId). 3. Backend извлекает из Bets (join с Events). 4. Отображение: событие, сумма, исход, статус (выигрыш/проигрыш).
...
- **Прецедент "Расчет результатов"** (как ранее, углубленно):
  - Актер: Администратор.
  - Предусловия: Событие завершено; исход известен.
  - Основной поток: 1. Admin выбирает событие. 2. Вводит фактический исход (Angular форма). 3. API (/events/calculate). 4. Backend: query Bets, расчет выигрышей (for each bet: if match then balance += amount * coeff), транзакции в БД.
  - Альтернативный поток: Частичный расчет (ничья).
...
- **Прецедент "Редактирование события"**:
  ...
  - Основной поток: 1. Выбор события. 2. Редактирование полей (дата, коэффициенты). 3. API (/events/update). 4. Backend обновляет в БД.
  - Альтернативный поток: Изменение коэффициентов — уведомление о существующих ставках.
  - Исключительный поток: Событие уже с ставками — ограниченное редактирование.
...
- **Прецедент "Генерация отчета по событиям"**:
  - Актер: Администратор.
  - Предусловия: Авторизован.
  - Основной поток: 1. Выбор параметров (дата, тип). 2. API (/reports/generate). 3. Backend агрегирует данные (SQL queries: sum bets, avg coeff). 4. Отображение в таблице или экспорт PDF (Angular).
  - Альтернативный поток: Отчет по пользователям.
  - Исключительный поток: Нет данных — пустой отчет.
  - Постусловия: Отчет сгенерирован.
  - Расширения: Графики (Chart.js в Angular).
...
- **Клиентская часть (Frontend)**: Angular — одностраничное приложение (SPA) для динамического интерфейса. Компоненты: страницы аутентификации, событий, ставок. Взаимодействие с backend через REST API (HTTP requests с Angular HttpClient).

- **Серверная часть (Backend)**: Java Jakarta EE — для бизнес-логики. Используем EJB для сессионных бинов (управление транзакциями), JPA (Hibernate) для ORM с PostgreSQL. Сервер приложений — WildFly или GlassFish (согласовать с преподавателем; WildFly легковесный для helios). RESTful сервисы с JAX-RS.
...
**Согласованные технологии (с преподавателем)**: Backend — Java Jakarta EE (JAX-RS, JPA); Frontend — Angular; DB — PostgreSQL. Дополнительно: Maven для сборки, Git для версионного контроля. Если нужно, добавить Spring Boot как альтернативу, но придерживаться Jakarta EE для enterprise-фокуса.