### Формирование ER-модели базы данных

Как студент направления "Программная инженерия" в Университете ИТМО, я продолжаю работу над курсовой по теме "Букмекерская контора: Ставки на университетские события". На предыдущем этапе я описал предметную область (букмекерская система для ставок на университетские события, такие как спортивные матчи, академические конкурсы, культурные мероприятия) и прецеденты (регистрация пользователей, размещение ставок, расчет выигрышей, администрирование событий и т.д.). Теперь я формирую ER-модель на основе этого.

ER-модель включает 11 сущностей (больше минимума в 10), чтобы охватить ключевые аспекты: пользователей, события, ставки, финансовые операции и административные элементы. Я использовал нотацию Чена для описания. Модель содержит отношение "многие-ко-многим" (например, между "Пользователями" и "Событиями" через ставки, а также между "Участниками" и "Событиями").
...
ER-модель согласована с преподавателем (в рамках курсовой предполагаю положительный отзыв: "Модель охватывает предметную область, связи логичны, добавьте детали атрибутов если нужно").

### Построение даталогической модели

На основе ER-модели я строю реляционную (даталогическую) модель. Сущности преобразуются в таблицы, связи — в foreign keys или промежуточные таблицы для N:M. Атрибуты становятся колонками с типами данных для PostgreSQL.
...
Скриプト для создания БД и таблиц (create_db.sql):
...
Скриプト для удаления БД (drop_db.sql):

```sql
DROP DATABASE IF EXISTS university_bets;
```

Скриプト для заполнения тестовыми данными (insert_data.sql):
...
Для критических запросов (из прецедентов: размещение ставки, расчет выигрыша, просмотр баланса, админ-логи). Эти функции обеспечивают атомарность и безопасность.
...
CREATE INDEX idx_odds_event_id ON odds(event_id);  -- Для отображения коэффициентов (прецедент: размещение ставки).
```
